...
class Classification:
    """What the classifier decided, plus why.
...
def classify_claim(
    ...
    """Classify a claim into (Tier, Magnitude) with an audit reason.
    ...
    tier, tier_reason = _classify_tier(content_lower, kt_lower, src_lower)
    magnitude, mag_reason = _classify_magnitude(content_lower, explicit_magnitude)
...
def _classify_tier(content_lower: str, kt_lower: str, src_lower: str) -> tuple[Tier, str]:
    """Apply tier rules in order; first match wins. Returns (tier, reason)."""
    # Rule 1: PATTERN knowledge type is definitionally PATTERN tier.
    if kt_lower == "pattern":
        return Tier.PATTERN, "rule-1: knowledge_type=PATTERN"
    ...
    if kt_lower in _OUTCOME_TYPES:
        return Tier.OUTCOME, f"rule-3: knowledge_type={kt_lower} (outcome-class)"

    # Rule 4: Content pattern keywords.
    for kw in _PATTERN_KEYWORDS:
        if kw in content_lower:
            return Tier.PATTERN, f"rule-4: content contains pattern-keyword '{kw}'"
    ...
    # above or below.
    return Tier.OUTCOME, "rule-6: default (no tier signal matched)"


def _classify_magnitude(
    content_lower: str,
    explicit: ClaimMagnitude | None,
) -> tuple[ClaimMagnitude, str]:
    """Apply magnitude rules. Returns (magnitude, reason)."""
    if explicit is not None:
        return explicit, f"explicit: caller passed {explicit.name}"
    ...
    for kw in _TRIVIAL_KEYWORDS:
        if kw in content_lower:
            return ClaimMagnitude.TRIVIAL, f"keyword '{kw}' -> TRIVIAL"

    for kw in _LOAD_BEARING_KEYWORDS:
        if kw in content_lower:
            return ClaimMagnitude.LOAD_BEARING, f"keyword '{kw}' -> LOAD_BEARING"

    return ClaimMagnitude.NORMAL, "default: no magnitude signal matched -> NORMAL"
