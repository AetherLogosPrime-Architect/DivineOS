...
def _clear_db_tables(db_path: Path, tables: list[str]) -> dict[str, int]:
    ...
    for table in tables:
        try:
            cur = conn.execute(f"SELECT COUNT(*) FROM {table}")  # nosec B608 — table from module-constant TABLES list, not user input
            count = cur.fetchone()[0]
            if count > 0:
                conn.execute(f"DELETE FROM {table}")  # nosec B608 — same closed table set
                removed[table] = count
