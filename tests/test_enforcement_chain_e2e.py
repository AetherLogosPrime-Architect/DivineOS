...
class TestCorrectionChain:
    ...
    Updated 2026-05-08 for the disclose-then-escalate redesign
    (pre-reg prereg-75c900fe). The compass-required gate now uses
    soft-advise for the first ESCALATION_THRESHOLD-1 fires and
    hard-deny only at/above the threshold. Per-turn dedup
    (PER_TURN_DEDUP_SECONDS) prevents same-marker re-firing within
    ~30s; tests need to either reset last_advised_ts or patch
    should_dedup_within_turn to simulate distinct turns.
    """
