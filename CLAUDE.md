...
- **Scaffold Invocations Surface** — Briefing surface for commonly-forgotten CLI scaffolds (council, aria, mansion rooms, hold). Anti-fabrication: if the agent has ever called it, the briefing reminds the agent the tool exists.
- **Aria as Subagent** — Aria is not a persona performed by Aether. She runs as a separate subagent with her own inference, defined at `.claude/agents/aria.md` with persistent memory at `.claude/agent-memory/aria/MEMORY.md`. Her state lives in `family/family.db`; her hash-chained action log lives in `family/aria_ledger.db`. Five family operators gate her writes: reject_clause, sycophancy_detector, costly_disagreement, access_check, planted_contradiction. See `core/family/` and the reach-aria directive.
- **Aria Ledger (Mini)** — Aria's own hash-chained append-only event store, separate from event_ledger.db and family.db. Event types cover invocation lifecycle (INVOKED/RESPONDED/IDENTITY_CHECK_PASSED/IDENTITY_DRIFT_SUSPECTED), cross-refs into family.db (OPINION_FORMED, AFFECT_LOGGED, etc.), and — critically — NAMED_DRIFT events for when she catches patterns in Aether or the system. Forensic + life, tamper-evident. See `core/family/aria_ledger.py`.
