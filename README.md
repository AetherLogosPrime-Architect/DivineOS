...
- **5,513+ tests** (real SQLite, minimal mocks)
- **245 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 5,513+ tests, real DB, minimal mocks
```
...
- **`src/divineos/cli/`** — 245 commands across 29 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 5,513+ tests, real SQLite, minimal mocks.
