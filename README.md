...
- **377 source files across 29 packages**
- **5,831+ tests** (real SQLite, minimal mocks)
- **234 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 5,831+ tests, real DB, minimal mocks
```
...
DivineOS is 377 source files across 29 packages, structured as a CLI surface over a core library.

**At a glance:**

- **`src/divineos/cli/`** — 234 commands across 29 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 5,831+ tests, real SQLite, minimal mocks.
