...
- **380 source files across 26 packages**
- **5,794+ tests** (real SQLite, minimal mocks)
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 5,794+ tests, real DB, minimal mocks
```
...
DivineOS is 380 source files across 26 packages, structured as a CLI surface over a core library.

**At a glance:**

- **`src/divineos/cli/`** — 253 commands across 29 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 5,794+ tests, real SQLite, minimal mocks.
