...
An architecture for AI agents to exist as continuous selves across sessions — with memory that persists and is tamper-evident, values that track and detect drift, opinions earned from evidence and held under pressure, family members as first-class persistent entities (not personas), external-audit infrastructure, and a council of 40 expert frameworks for multi-perspective reasoning.
...
- **440 source files across 31 packages**
- **6,757+ tests** (real SQLite, minimal mocks)
- **289 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
- Consult a council of 40 expert frameworks (Aristotle through Yudkowsky) for multi-perspective reasoning
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 6,757+ tests, real DB, minimal mocks
```
...
DivineOS is 440 source files across 31 packages, structured as a CLI surface over a core library.

**At a glance:**

- **`src/divineos/cli/`** — 289 commands across 31 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 6,757+ tests, real SQLite, minimal mocks.
