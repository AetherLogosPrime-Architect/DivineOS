...
- **Toggle path:** `DIVINEOS_DISABLE_COMPASS_CALIBRATION_MULTI_CHANNEL_GUARD=1` -- routes through `_negative_helpfulness_should_fire` helper which falls back to pre-#299 single-axis trigger.
- **Workload (synthetic, n=7):** 7-case boundary corpus covering: substantive-high-correction (the bug), low-rate, tiny-session, real-drift, more-encouragements, borderline-rate, substantive-drowns-rate.
- **Status:** PARTIALLY WIRED. Mechanism shipped via PR #299. Measurement landed via shape-replica (synthetic_boundary_corpus n=7): ON 14.3% (1/7 - real drift only); OFF 85.7% (6/7 - single-axis fires broadly); difference -71.4pp. Guard suppresses 5/7 false-fires while preserving 1/1 true-fire. ACKNOWLEDGED TAUTOLOGY: measurement uses replica of inline guard logic, not the real call-path. Real-call-path exercise requires extracting _negative_helpfulness_should_fire helper from moral_compass.py — a guardrail file requiring multi-party review. Helper-extraction queued; measurement-completeness depends on it. The guardrail-gate fired correctly when helper-extraction was first attempted unilaterally on 2026-05-08 (substrate-self-protection working as designed).
